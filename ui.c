...
const unsigned char fkeys_main[] = {
	...
	0x20, 0x0, 0x1, 0x9c, 0xd5, 0xcb, 0x45, 0xde, 0x40, 0x0, 0x2, 0x0, 0x0, 0x10, 0xba, 0x8c, 
	0x26, 0x14, 0x31, 0x6b, 0xb5, 0xcb, 0x5e, 0xbe, 0x40, 0x20, 0x2, 0xf, 0xe0, 0x17, 0xd6, 0xdc, 
	0x29, 0x22, 0x9, 0x6b, 0xb5, 0xc8, 0x47, 0x7e, 0x40, 0x70, 0x2, 0xf, 0xe0, 0x11, 0xee, 0xdc, 
	0x2f, 0x7f, 0x39, 0xd, 0xb5, 0xcb, 0x5e, 0xbe, 0x40, 0xf8, 0x2, 0x7, 0xc0, 0x17, 0xd6, 0xdc, 
	0x29, 0x22, 0x49, 0x6e, 0xb5, 0x8b, 0x5d, 0xdc, 0x41, 0xfc, 0x2, 0x3, 0x80, 0x17, 0xba, 0xd8, 
	0x29, 0x14, 0x39, 0x69, 0xd5, 0xb, 0x45, 0xd8, 0x41, 0xfc, 0x2, 0x1, 0x0, 0x10, 0xba, 0xd0, 
	0x20, 0x0, 0x1, 0xff, 0xfe, 0xf, 0xff, 0xf0, 0x40, 0x0, 0x2, 0x0, 0x0, 0x1f, 0xff, 0xe0, 
};
...
unsigned char asciidlg_custom(void) {
	...
	c = InputNumber("ASCII code (hex)", 2, IN_MODE_HEX);
...
void setupmenu(void) {
	...
	while (1) {
		...
		if (SETUP_NUMKEYS > 7) { //not handled yet
			return;
		}
		...
		if (setuparr[SETUP_KEY_THEME] == SETUP_THEME_DARK) {
			Bdisp_AreaReverseVRAM(0, 0, 127, 63); //invert whole screen;
		}
		...
		if (key == KEY_CTRL_F1) {
			switch(sel) { //which one is edited currently?
				case SETUP_KEY_PROCESC:
					setuparr[SETUP_KEY_PROCESC] = SETUP_PROCESC_ON;
					break;
				case SETUP_KEY_THEME:
